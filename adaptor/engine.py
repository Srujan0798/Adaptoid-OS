...
3. Init **Agile SDLC** tasks:
   `python3 <adaptoid>/conductor/conductor.py init-wave --project . --sdlc`
4. Loop: PLAN → DESIGN → BUILD → TEST → preflight SHIP → rewrite HANDOFF
5. Use your **host** for plan mode, subagents, skills, MCP, git, terminal
6. Before claiming done: `bash orchestrator/scripts/preflight.sh .`
...
protocols/sdlc-loop.md  efficient SDLC gates (not theater)
...
def copy_sdlc_docs(output_dir: Path):
    """Copy thin SDLC + host-capability guides into the project."""
    proto_dst = output_dir / "protocols"
    proto_dst.mkdir(parents=True, exist_ok=True)
    for rel in ("protocols/sdlc-loop.md", "core/HOST-CAPABILITIES.md"):
        src = OS_SETUP_ROOT / rel
        if not src.exists():
            continue
        if rel.startswith("core/"):
            dst = output_dir / "HOST-CAPABILITIES.md"
        else:
            dst = proto_dst / src.name
